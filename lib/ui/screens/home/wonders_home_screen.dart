                            ...
                            Semantics(
                              ...
                              child: Builder(
                                builder: (context) {
                                  ...
                                  // Determine if small text adjustments are needed
                                  bool smallText =
                                      [WonderType.christRedeemer, WonderType.colosseum].contains(currentWonder?.type);
                                  if (smallText) {
                                    textStyle = textStyle.copyWith(fontSize: 56 * $styles.scale);
                                  }

                                  // Text styling logic for "Choose the theme"
                                  final title = "choose the theme";
                                  List<String> pieces = title.split(' ');

                                  // Helper function to create styled TextSpan
                                  TextSpan buildTextSpan(String text) {
                                    ...
                                    return TextSpan(
                                      ...
                                      style:
                                          useSmallText ? textStyle.copyWith(fontSize: 20 * $styles.scale) : textStyle,
                                    );
                                  }

                                  // Define text shadows if enabled
                                  // List<Shadow> shadows = enableShadows ? $styles.shadows.textSoft : [];

                                  // Create a RichText widget for the styled text
